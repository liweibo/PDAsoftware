...
public class MyAdapter extends BaseAdapter {
    ...
    public List<FtpUtils.wxhFile> listDeal(List<FtpUtils.wxhFile> list) {
        int tmpSize = list.size();
        if (tmpSize > 0) {
            for (int i = 0; i < tmpSize; i++) {
                if (list.get(i).filename.equals("./") || list.get(i).filename.equals("../")) {
                    list.remove(i);
                }
            }
        }
                                                 ...
                                                 public void onCheckedChanged(CompoundButton compoundButton, final boolean b) {
                                                     new Thread(new Runnable() {
                                                         ...
                                                         public void run() {
                                                             ...
                                                             if (!b) {
                                                                 //没选中的item，则从记录中删除对应
                                                                 if (selectFileActivity.currentFiles.size() > 0) {
                                                                     itemChooseData.removeFilePath(position);

                                                                     itemChooseData.removePathAndisfile(position);
                                                                 ...
                                                                 if (selectFileActivity.currentFiles.size() > 0) {
                                                                     System.out.println("测试被勾选的文件/文件夹" + selectFileActivity.currentFiles.get(position).filePath);
                                                                     itemChooseData.addFilePath(selectFileActivity.currentFiles.get(position).filePath);

                                                                     itemChooseData.addPathAndIsfile(selectFileActivity.currentFiles.get(position).filePath,
                                                                             selectFileActivity.currentFiles.get(position).isFile);

                                                                     itemChooseData.addFileName(selectFileActivity.currentFiles.get(position).filename);
                                                                     String isfile = selectFileActivity.currentFiles.get(position).isFile ? "file" : "dir";
                                                                 }
                                                             }
                ...
                if (selectFileActivity.currentFiles.size() > 0) {
                    ...
                    new Thread(new Runnable() {
                        ...
                        public void run() {
                            ...
                            itemChooseData.removePathAndisfile(finalI);
                ...
                if (selectFileActivity.currentFiles.size() > 0) {
                    ...
                    new Thread(new Runnable() {
                        ...
                        public void run() {
                            ...
                            itemChooseData.addPathAndIsfile(selectFileActivity.currentFiles.get(finalI).filePath,
                                    selectFileActivity.currentFiles.get(finalI).isFile);
